...
%\textcolor{blue}{\textbf{Dans le papier, ils disent qu'ils ont 174 images, j'en ai 245 dans le dossier.}}
%The X-ray images are from an retrospective multi-centric study that was conducted at
%three French research hospitals (Amiens, Orleans and Cochin in Paris). ({\sc details necessaires?})
%Patients were recruited from a program originated by the hospital of
%Orleans between Nov. 2004 and Feb. 2006. 
...
%This step is performed by sustracting the trend which is represented by a plane obtained via a mean-square linear regression.
and then 
%Let $\alpha \in (0,1)$ and $t_{\alpha} = F^{-1}_{X}(\alpha)$ be the associated quantile of the image $X$. We 
...
%compute the horizontal and vertical perimeter of each thresholded image, thus obtaining for each level $t_{\alpha}$, $(P^{(1)}_{i}(t_{\alpha}))_{i \leq 245}$, count
...
%Which occurs when the distribution of the oriented perimeter is not the same, which appears to be the case in an anisotropic framework.
...
%Apparently, these images were subject to a $\pi/2$-angle rotation. 
...
\end{align*}
\end{proof}
This result extends the one given in \cite{HermineAgnes} (Proposition 3) where the expected perimeter is computed in the case $\rho(e_i) > 0$. Using the former expression, we are also able to obtain an analytic expression for the first moment.
\begin{Proposition}(Analytic expression for the first moment of the oriented perimeter)
Considering again the former expression, we get that 
\label{analytic expression}
  \begin{align}
  \label{propEquationFisrtmoment2}
...
\end{figure}
\textcolor{red}{mettre une remarque qui dit que l'existence de la densite spectral nous dit que si les les points sont distincts la covariance est inversible . Donc la somme se decompose en : j'ai 2 points, 3 ou 4 points, toruver une ref pour justifier tout ca} \\
...
$\left\{
 \begin{array}{rlc}
 \sigma^{\scriptscriptstyle (-)}_{1} & = &  \hspace{-6.9cm} \sqrt{2(1-\rho(e_1))} \\
 \sigma^{\scriptscriptstyle (+)}_{1} & = & \sqrt{2(1+\rho(e_1))} \hspace{2cm} \text{with $\sigma^{\scriptscriptstyle (-)}_{1} > 0,  \; \sigma^{\scriptscriptstyle (+)}_{1} > 0 $, and} \\
 \end{array}\right.$\\
 $\left\{
 \begin{array}{rlc}
   \pi^{\scriptscriptstyle(-, i)}_{U}(x) & = & \hspace{-8cm} \frac{1}{\sigma^{\scriptscriptstyle (-)}_{1}}\left(\Delta \rho(x+e_i) -\Delta \rho(x)\right)  \\
   \pi^{\scriptscriptstyle(-, i)}_{V}(x) & = & \hspace{-8cm} \frac{1}{\sigma^{\scriptscriptstyle (+)}_{1}}\left(S\rho(x+e_i)-S\rho(x)\right)  \\
   \pi^{\scriptscriptstyle(-, i)}_{W}(x)& = & \left(2(1 - \rho(e_{i})) - \pi^{\scriptscriptstyle(-, i)}_{U}(x)^{2} - \pi^{\scriptscriptstyle(-, i)}_{V}(x)^{2}\right)^{\scriptscriptstyle (1/2)} \text{with also $\pi^{\scriptscriptstyle(-, i)}_{W}(x) > 0$, and}, \end{array}\right.$   \\ $\left\{
 \begin{array}{rlc}
   \pi^{\scriptscriptstyle(+, i)}_{U}(x) & = & \hspace{-9.5cm} \frac{1}{\sigma^{\scriptscriptstyle (-)}_{1}}\left(\Delta \rho(x) + \Delta \rho(x+e_i)\right) \\
    \pi^{\scriptscriptstyle(+, i)}_{V}(x) & = & \hspace{-9.5cm} \frac{1}{\sigma^{\scriptscriptstyle (+)}_{1}}\left(S \rho(x) + S\rho(x+e_i)\right) \\
     \pi^{\scriptscriptstyle(+, i)}_{W}(x) & = & \hspace{-6cm} \frac{-1}{\pi^{\scriptscriptstyle(-, i)}_{W}(x)}\left(\pi^{\scriptscriptstyle(-, i)}_{U}(x)\pi^{\scriptscriptstyle(+, i)}_{U}(x) + \pi^{\scriptscriptstyle(-, i)}_{V}(x)\pi^{\scriptscriptstyle(+, i)}_{V}(x)\right) \\
   \pi^{\scriptscriptstyle(+, i)}_{Z}(x)& = & \left(2(1 + \rho(e_i)) - \pi^{\scriptscriptstyle(+, i)}_{U}(x)^{2} - \pi^{\scriptscriptstyle(+, i)}_{V}(x)^{2} - \pi^{\scriptscriptstyle(+, i)}_{W}(x)^{2}\right)^{\scriptscriptstyle (1/2)} \text{and finally, $\pi^{\scriptscriptstyle(+, i)}_{Z}(x) > 0$}.
\end{array}\right.$  \\
then,
{\small
 \begin{align}
...
\begin{figure}[H]
  \centering
    {\includegraphics[scale=0.28]{Figurepi411.pdf}} \hspace{-1cm}
      {\includegraphics[scale=0.28]{Figurepi410.pdf}}
      \end{figure}
      \vspace{-3cm}
      \begin{figure}[H]
      \hspace{-0.5cm}
        \centering
    {\includegraphics[scale=0.28 ]{TestaniRatiopvaluepi4}}
    % {\includegraphics[scale=0.25]{TestaniRatiopvaluepi410.pdf}}
    % \vspace{-3cm}
...
\begin{figure}[H]
  \centering
    {\includegraphics[scale=0.4]{Boxplotimageretourne.pdf}}
    \hspace{0.2cm}
...
\begin{figure}[H]
  \centering
    {\includegraphics[scale=0.38]{Testalphadonnereel.pdf}}
    \hspace{0.2cm}
...
% \begin{figure}[H]
%   \centering
%     {\includegraphics[scale=0.4]{rho.pdf}}
%     \hspace{0.2cm}
%  \caption{For $\alpha \in [0.15, 0.99]$, we represent the $p_{value}$ of the test computed for different levels $q_{alpha}$ for real thresholded images of bone X-rays.}
% % \label{fig}
% \end{figure}
% \begin{figure}[H]
%   \centering
%     {\includegraphics[scale=0.4]{boxplotrho.pdf}}
%     \hspace{0.2cm}
%  \caption{For $\alpha \in [0.15, 0.99]$, we represent the $p_{value}$ of the test computed for different levels $q_{alpha}$ for real thresholded images of bone X-rays.}
% % \label{fig}
% \end{figure}
\begin{figure}[H]
  \centering
    {\includegraphics[scale=0.4]{rhoss.pdf}}
    \hspace{0.2cm}
