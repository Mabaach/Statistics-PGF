...
Let $m$ be an integer with $m \geq 2$. Without loss of generality, we consider our observation window as the unit square $S = [0,m]^{2}$ and  we divide our window into $m^{2}$ pairwise disjoint squares. We denote by $\mathbb{G}_{m} := \text{\larger[1]{$[$}}0, m \text{\larger[1]{$)$}}^{2} \cap \mathbb{Z}^{2} $ the set of point in $S$ with interger coordinates and for $x \in \mathbb{G}_{m}$, we define $C_{m}(x)$ as
\begin{equation*}
...
We assume that $\left(X_x \right)_{x \in \mathbb{G}_{m}}$ is a stationary Gaussian centered with unit variance random field let us denote \linebreak by  $\rho(e_i) = \text{Cov}\left(X_{\scriptscriptstyle 0}, X_{e_i}\right) \in~[-1, 1]$. The expected value of the perimeter in \eqref{eq:6} is  given \linebreak by $$\mathbb{E}\left(\mathcal{P}_{m}(t)\right) = m(m-1)\sum_{1}^{2}\mathbb{E}\left(f^{(i)}(t) \right)$$  and
\begin{itemize}
...
\begin{align*}
\left\{ x \in \mathbb{G}_{m}; w_{i}(x) \in \mathbb{E}^{\scriptscriptstyle  (i)}_{m}\right\} & = \left\{x \in \llbracket 0, m-1 \rrbracket^{2}, (x, x+e_i)\subset (0,m)^{2} \right\} \\ 
...
\draw (4,0) node[below]{$\frac{b+a}{2}$};
...
\draw (8, 0.6) -- (7.75,0.7);
\draw (8, 0.6) -- (7.75,0.5);
\node[text width= 1.6cm, font=\large] at (4,1) {$\frac{|b-a|}{2}$};
\node[text width= 0.7cm, font=\large] at (3,-1) {$|t -~\frac{b+a}{2}|$};
\end{tikzpicture}
\end{figure}
...
\begin{equation*}
 \text{if} \hspace{0.5cm} \tilde{\rho}(e_i) \leq \bar{\rho}(e_i) \hspace{0.5cm} \text{then} \hspace{0.5cm}\mathbb{E}\left(\mathcal{P}^{\scriptscriptstyle (i)}_{m, \tilde{\rho}}(t)\right) \geq \mathbb{E}\left(\mathcal{P}^{\scriptscriptstyle (i)}_{m, \bar{\rho}}(t)\right).
\end{equation*}
Where $\mathcal{P}^{\scriptscriptstyle (i)}_{m, \tilde{\rho}}$ is the perimeter with respect of the orientation $i$ associated to the Gaussian field of covariance $\tilde{\rho}$. 
\end{remark}
\begin{proof}
Let $t \in \mathbb{R}$ and $X$, $Y$ two centered with unit variance Gaussian random fields on $\mathbb{Z}^{2}$ of covariance stucture respectively $\tilde{\rho}$ and $\bar{\rho}$. We assume that $\tilde{\rho}(e_i) \leq \bar{\rho}(e_i)$, and consider the Gaussian vectors $\left(X_{0}, X_{e_1}\right)$ and $\left(Y_{0}, Y_{e_1}\right)$, $\tilde{\rho}(e_i) \leq \bar{\rho}(e_i)$ implies that $\mathbb{E}\left(X_{0}X_{e_1}\right) < \mathbb{E}\left(Y_{0}Y_{e_1}\right)$, we can write that
\begin{align*}
\mathbb{P}\left(Y_{\scriptscriptstyle 0} < t \leq  Y_{e_1}\right) = \mathbb{P}\left(Y_{\scriptscriptstyle 0} < t\right) - \mathbb{P}\left(Y_{\scriptscriptstyle 0} < t,  Y_{e_1} < t\right),
\end{align*}
according to Slepian's lemma, $\mathbb{P}\left(X_{\scriptscriptstyle 0} < t,  X_{e_1} < t\right) \leq \mathbb{P}\left(Y_{\scriptscriptstyle 0} < t,  Y_{e_1} < t\right)$, \linebreak besides,~$\mathbb{P}\left(Y_{\scriptscriptstyle 0} < t\right)=\mathbb{P}\left(X_{\scriptscriptstyle 0} < t\right)$ thus, 
\begin{align*}
\mathbb{P}\left(Y_{\scriptscriptstyle 0} < t \leq  Y_{e_1}\right) \leq \mathbb{P}\left(X_{\scriptscriptstyle 0} < t \leq  X_{e_1}\right)
\end{align*}
and,
\begin{align*}
\mathbb{P}\left(\min\left(Y_{\scriptscriptstyle 0}, Y_{e_1}\right) < t \leq \max\left(Y_{\scriptscriptstyle 0}, Y_{e_1}\right)\right)  \leq \mathbb{P}\left(\min\left(X_{\scriptscriptstyle 0}, X_{e_1}\right) < t \leq \max\left(X_{\scriptscriptstyle 0}, X_{e_1}\right)\right) 
\end{align*}
thus, 
\begin{align*}
\mathbb{E}\left(\mathcal{P}^{\scriptscriptstyle (i)}_{m, \tilde{\rho}}(t)\right) \geq \mathbb{E}\left(\mathcal{P}^{\scriptscriptstyle (i)}_{m, \bar{\rho}}(t)\right).
\end{align*}
\end{proof}
\begin{remark}
Considering the contraposition of this result, we get the following 
\begin{equation*}
 \text{if} \hspace{0.5cm}\mathbb{E}\left(\mathcal{P}^{\scriptscriptstyle (i)}_{m, \tilde{\rho}}(t)\right) \neq \mathbb{E}\left(\mathcal{P}^{\scriptscriptstyle (i)}_{m, \bar{\rho}}(t)\right)   \hspace{0.5cm} \text{then}  \hspace{0.5cm} \tilde{\rho}(e_i) \neq \bar{\rho}(e_i) \hspace{0.5cm}.
\end{equation*} 
\end{remark}
...
Figure \ref{configurations} below shows the five types of configurations depending on the orientation $i$ and position on the grid $x$. 
\begin{figure}[H]
\begin{center}
...
\end{align*}}Let us assume that $|\rho(e_1)| < 1$, the Cholesky decomposition of $\tilde{\Sigma}_{i}(x)$ allows us to get the expression of the Gaussian vector $\left(\Delta^{\scriptscriptstyle (1)}_{\scriptscriptstyle 0},S^{\scriptscriptstyle (1)}_{\scriptscriptstyle 0},\Delta^{\scriptscriptstyle (i)}_{\scriptscriptstyle x},S^{\scriptscriptstyle (i)}_{\scriptscriptstyle x} \right)$ in a new basis $\left(U,V,W,Z\right)$ with $\left(U, V, W, Z\right) \sim \mathcal{N}\left(0,I_{4}\right)$. We get the following decomposition \\ 
$\left\{
  \begin{array}{rlc}
...
\begin{Lemma}
Let $r(x) = \max\left(|\rho(x)|, |\rho(x + e_i)|, |\rho(x-e_1)|, |\rho(x +e_i - e_1)| \right)$, there exist a constant $C \in \mathbb{R}^{+}$, for all $x \in \mathbb{G}_{m}$
\begin{align*}
...
\end{Lemma}
\begin{proof}
Let us begin by reconsidering the previous quantities and 
\begin{table}[H]
\centering 
\begin{tabularx}{\textwidth}{|P{3.5cm}|P{11.85cm}|}
\hline
Variable & upper bound \\ \hline
$\left|\pi^{\scriptscriptstyle(-, i)}_{U}(x)\right|$ & $\frac{4r(x)}{\sigma^{\scriptscriptstyle (-)}_{1}}$ \\  \hline
$\left|\pi^{\scriptscriptstyle(-, i)}_{V}(x)\right|$ & $\frac{4r(x)}{\sigma^{\scriptscriptstyle (+)}_{1}}$  \\  \hline
$\left|\pi^{\scriptscriptstyle(-, i)}_{W}(x) - \sigma^{(-)}_{i}\right|$ & $ \frac{16r^{2}(x)}{\sigma^{(-)}_{i}}\left(\frac{1}{(\sigma^{(-)}_{i})^{2}} + \frac{1}{(\sigma^{(+)}_{i})^{2}} \right)$ \\ 
\hline
$|\pi^{\scriptscriptstyle(+, i)}_{U}(x)|$ &  $\frac{4r(x)}{\sigma^{\scriptscriptstyle (-)}_{1}}$   \\ \hline
$|\pi^{\scriptscriptstyle(+, i)}_{V}(x)|$ &   $\frac{4r(x)}{\sigma^{\scriptscriptstyle (+)}_{1}}$  \\ \hline 
$ \left|\pi^{\scriptscriptstyle(+, i)}_{W}(x)\right| $ & $16r(x)^{2}\left(\sigma^{(-)}_{i} -  \frac{16r^{2}(x)}{\sigma^{(-)}_{i}}\left(\frac{1}{(\sigma^{(-)}_{i})^{2}} + \frac{1}{(\sigma^{(+)}_{i})^{2}} \right)\right)^{-1}\left(\frac{1}{(\sigma^{(-)}_{i})^{2}} + \frac{1}{(\sigma^{(+)}_{i})^{2}} \right)$\\ \hline
$\left|\pi^{\scriptscriptstyle(+, i)}_{Z}(x) - \sigma^{(+)}_{i}\right|$ & $\frac{16r^{2}(x)}{\sigma^{(+)}_{i}}\left(\frac{1}{(\sigma^{(-)}_{i})^{2}} + \frac{1}{(\sigma^{(+)}_{i})^{2}} +  \frac{\pi^{\scriptscriptstyle(+, i)}_{W}(x)^{2}}{16r^{2}(x)} \right)$\\\hline
\end{tabularx}
\end{table}
...
& \leq Cr(x)
\end{align*}
Considering the series $\sum_{x \in \mathbb{Z}^{2}}\left(\mathbbm{1}_{x_{1} \in [2-m, m-2]}\mathbbm{1}_{x_{2} \in [1-m, m-1]} \frac{(m-|x_2|)(m-|x_1| -1)}{m^2} \text{Cov}\left(f_{t}^{\scriptscriptstyle(1)}(0), f_{s}^{\scriptscriptstyle(i)}(x) \right) \right)$
The limit of the general term as $m$ goes to infinity is
\begin{align*}
\lim_{m \to \infty}\frac{(m-|x_2|)(m-|x_1| -1)}{m^2} \text{Cov}\left(f_{t}^{\scriptscriptstyle(1)}(0), f_{s}^{\scriptscriptstyle(i)}(x) \right)\mathbbm{1}_{x_{1} \in [2-m, m-2]\cap \mathbb{Z}}\mathbbm{1}_{x_{2} \in [1-m, m-1]\cap \mathbb{Z}} \\
& \hspace{-7cm} =\text{Cov}\left(f_{t}^{\scriptscriptstyle(1)}(0), f_{s}^{\scriptscriptstyle(i)}(x) \right)\mathbbm{1}_{x \in \mathbb{Z}^{2}} 
\end{align*}
Besides, we just proved that for $m \in \mathbb{N}$, for all $x\in \mathbb{Z}^{2}$, $\text{Cov}\left(f_{t}^{\scriptscriptstyle(1)}(0), f_{s}^{\scriptscriptstyle(i)}(x)\right) \leq Cr(x)$, thus applying the dominated convergence theorem, $\lim_{m \to \infty}\text{Cov}\left(\frac{1}{m}\mathcal{P}_{m}^{\scriptscriptstyle (1)}(t), \frac{1}{m}\mathcal{P}_{m}^{\scriptscriptstyle (i)}(s) \right)$ exists.
\end{proof}
Now that we know more about how the covariance behaves, we can construct a Multi-directional multivariate CLT that will lead us to an isotropy test.
\vspace{-0.5cm}{}
...
\end{itemize} 
\begin{Theorem}\label{TCL}(Multi-directional multivariate CLT for r-threshold). We consider taht $X_{x \in \mathbb{G}_{m}}$ a centered stationary standard Gaussian, let $r$ be a positive integer, $m \geq 2$ and $t_{1}, \cdots,t_{r} \in \mathbb{R}$, then, under \textbf{$(A_{1})$} and \textbf{$(A_{2})$},
\begin{equation*}
...
\end{equation} 
% $\tilde{\sigma}^{2}(t) = \frac{2\left(\tilde{\sigma}_{\scriptscriptstyle 1,1}^{2}(t) \, - \, \tilde{\sigma}_{\scriptscriptstyle 1,2}^{2}(t)\right)}{\mu^{\star}_{2}(t)^2}$
\end{Proposition}
...
\begin{equation}
\label{phihat}
\hat{\phi}_{m}(t)= \mathbbm{1}_{\left\{ \frac{1}{\sqrt{\hat{\mathbb{V}\left(R_{m}(t)\right)}}}\left|\left(R_{m}(t) - 1 \right)\right| \geq q_{1- \frac{\alpha}{2}} \right\}}
\end{equation}
...
%  $\left\{
%  \begin{array}{rlc}
%  \pi_{U}(x) & = & \frac{1}{\sigma^{\scriptscriptstyle (-)}_{1}}\left(\rho(x-e_1) - \rho(x)\right) \\
%  \pi_{V}(x) & = &  \frac{1}{\sigma^{\scriptscriptstyle (+)}_{1}}\left(\rho(x-e_1) + \rho(x)\right)  \\
%  \pi_{W}(x)& = & \left(1 - \pi_{U}(x)^{2} - \pi_{V}(x)^{2}\right)^{\scriptscriptstyle (1/2)} \end{array}\right.$ 
...
&= \mathbbm{1}_{\left\{  S^{\scriptscriptstyle (1)}_{0} \leq 2s + \Delta^{(1)}_{0}\right\}} \mathbbm{1}_{\left\{ S_{x} \geq s+ \frac{1}{2}S^{\scriptscriptstyle (1)}_{0} + \frac{1}{2}\Delta^{\scriptscriptstyle (1)}_{0}\right\}} + \mathbbm{1}_{\left\{  2s + \Delta^{(1)}_{0} \leq S^{\scriptscriptstyle (1)}_{0} \right\}} \mathbbm{1}_{\left\{ S_{x} \leq  S^{\scriptscriptstyle (1)}_{0} \right\}} \\
\end{align*} 
% \begin{align*}
% \mathbbm{1}_{\left\{ S^{\scriptscriptstyle (1)}_{x} - S^{\scriptscriptstyle (1)}_{0} \geq 0 \right\}} \mathbbm{1}_{\left\{  - S^{\scriptscriptstyle (1)}_{x} + S^{\scriptscriptstyle (1)}_{0} \leq 2s + \Delta^{(1)}_{0} -  S^{\scriptscriptstyle (1)}_{x}  \leq S^{\scriptscriptstyle (1)}_{x} - S^{\scriptscriptstyle (1)}_{0}\right\}} & =\\
% & \hspace{-4.5cm} \mathbbm{1}_{\left\{ S^{\scriptscriptstyle (1)}_{x}  \geq S^{\scriptscriptstyle (1)}_{0}\right\}} \mathbbm{1}_{\left\{  S^{\scriptscriptstyle (1)}_{0} \leq 2s + \Delta^{(1)}_{0}\right\}}\mathbbm{1}_{\left\{ 2s + \Delta^{(1)}_{0}  + S^{\scriptscriptstyle (1)}_{0} \leq 2S^{\scriptscriptstyle (1)}_{x} \right\}}\\
% & \hspace{-5cm} = \mathbbm{1}_{\left\{  S^{\scriptscriptstyle (1)}_{0} \leq 2s + \Delta^{(1)}_{0}\right\}} \mathbbm{1}_{\left\{\pi^{\scriptscriptstyle(i)}_{W}(x) W \geq  - \pi^{\scriptscriptstyle(i)}_{U}(x)U + \left(\sigma^{(+)}_{1} - \pi^{\scriptscriptstyle(i)}_{V}(x)\right)V\right\}}\\
% & \hspace{1cm} \mathbbm{1}_{\left\{ 2s + \left(\sigma^{(-)}_{1} - 2\pi^{\scriptscriptstyle(i)}_{U}(x)\right)U  +  \left(\sigma^{(+)}_{1} - 2\pi^{\scriptscriptstyle(i)}_{V}(x)\right) \leq 2\pi^{\scriptscriptstyle(i)}_{W}(x) W \right\}} \\
% & \hspace{-5cm} = \mathbbm{1}_{\left\{  S^{\scriptscriptstyle (1)}_{0} \leq 2s + \Delta^{(1)}_{0}\right\}} \\
% & \hspace{-4cm} \mathbbm{1}_{\left\{\pi^{\scriptscriptstyle(i)}_{W}(x) W  \geq  \max(- \pi^{\scriptscriptstyle(i)}_{U}(x)U + \left(\sigma^{(+)}_{1} - \pi^{\scriptscriptstyle(i)}_{V}(x)\right)V, s + \left(\frac{1}{2}\sigma^{(-)}_{1}- \pi^{\scriptscriptstyle(i)}_{U}(x)\right)U  +  \left(\frac{1}{2}\sigma^{(+)}_{1}- \pi^{\scriptscriptstyle(i)}_{V}(x)\right) \right\}}\\
% \end{align*}

% \begin{align*}
% \mathbbm{1}_{\left\{ S^{\scriptscriptstyle (1)}_{x} - S^{\scriptscriptstyle (1)}_{0} \leq 0 \right\}} \mathbbm{1}_{\left\{  S^{\scriptscriptstyle (1)}_{x} - S^{\scriptscriptstyle (1)}_{0} \leq 2s + \Delta^{(1)}_{0} -  S^{\scriptscriptstyle (1)}_{x}  \leq -S^{\scriptscriptstyle (1)}_{x} + S^{\scriptscriptstyle (1)}_{0}\right\}} & =\\
% & \hspace{-4.5cm} \mathbbm{1}_{\left\{ S^{\scriptscriptstyle (1)}_{x}  \leq S^{\scriptscriptstyle (1)}_{0}\right\}} \mathbbm{1}_{\left\{  S^{\scriptscriptstyle (1)}_{0} \geq 2s + \Delta^{(1)}_{0}\right\}}\mathbbm{1}_{\left\{ 2s + \Delta^{(1)}_{0}  + S^{\scriptscriptstyle (1)}_{0} \geq 2S^{\scriptscriptstyle (1)}_{x} \right\}}\\
% & \hspace{-5cm} = \mathbbm{1}_{\left\{  S^{\scriptscriptstyle (1)}_{0} \geq 2s + \Delta^{(1)}_{0}\right\}} \mathbbm{1}_{\left\{\pi^{\scriptscriptstyle(i)}_{W}(x) W \leq  - \pi^{\scriptscriptstyle(i)}_{U}(x)U + \left(\sigma^{(+)}_{1} - \pi^{\scriptscriptstyle(i)}_{V}(x)\right)V\right\}}\\
% & \hspace{-1.7cm} \mathbbm{1}_{\left\{ 2s + \left(\sigma^{(-)}_{1} - 2\pi^{\scriptscriptstyle(i)}_{U}(x)\right)U  +  \left(\sigma^{(+)}_{1} - 2\pi^{\scriptscriptstyle(i)}_{V}(x)\right) \geq 2\pi^{\scriptscriptstyle(i)}_{W}(x) W \right\}} \\
% & \hspace{-5cm} = \mathbbm{1}_{\left\{  S^{\scriptscriptstyle (1)}_{0} \geq 2s + \Delta^{(1)}_{0}\right\}} \\
% & \hspace{-4cm} \mathbbm{1}_{\left\{\pi^{\scriptscriptstyle(i)}_{W}(x) W \leq  \min(- \pi^{\scriptscriptstyle(i)}_{U}(x)U + \left(\sigma^{(+)}_{1} - \pi^{\scriptscriptstyle(i)}_{V}(x)\right)V, s + \left(\frac{1}{2}\sigma^{(-)}_{1}- \pi^{\scriptscriptstyle(i)}_{U}(x)\right)U  +  \left(\frac{1}{2}\sigma^{(+)}_{1}- \pi^{\scriptscriptstyle(i)}_{V}(x)\right) \right\}}\\
% \end{align*}
...
& \hspace{-10cm} \mathbb{E}\left(\mathbbm{1}_{\left\{|2t - \sigma^{(+)}_{1}V \leq  \sigma^{(-)}_{1}U|\right\}}\mathbbm{1}_{\left\{ \sigma^{(+)}_{1}V  \leq 2s + \sigma^{(-)}_{1}U\right\}} \left(1 - \Phi_{\pi^{\scriptscriptstyle(i)}_{W}(x) }\left(s + \left(\frac{1}{2}\sigma^{(+)}_{1}- \pi^{\scriptscriptstyle(i)}_{U}(x)\right)U  +  \left(\frac{1}{2}\sigma^{(+)}_{1}- \pi^{\scriptscriptstyle(i)}_{V}(x)\right)V\right)\right)\right)\\
& \hspace{-10cm}+ \mathbb{E}\left(\mathbbm{1}_{\left\{|2t - \sigma^{(+)}_{1}V \leq  \sigma^{(-)}_{1}U|\right\}}\mathbbm{1}_{\left\{ \sigma^{(+)}_{1}V  \geq 2s + \sigma^{(-)}_{1}U\right\}}\Phi_{\pi^{\scriptscriptstyle(i)}_{W}(x) }\left(- \pi^{\scriptscriptstyle(i)}_{U}(x)U + \left(\sigma^{(+)}_{1} - \pi^{\scriptscriptstyle(i)}_{V}(x)\right)V\right) \right)\\\
\end{align*}
}
The second configuration would be to have
{\small
\begin{align*}
...
\begin{align*}
\mathbb{E}\left(\mathbbm{1}_{\left\{\min(X_{\scriptscriptstyle 0}, X_{e_1}) \leq t \leq \max(X_{\scriptscriptstyle 0}, X_{e_1})\right\}}\mathbbm{1}_{\left\{\min(X_{x}, X_{0}) \leq s \leq \max(X_{x}, X_{0}) \right\}}\right) = \\
& \hspace{-10cm}\mathbb{E}\left(\mathbbm{1}_{\left\{|2t - \sigma^{(+)}_{1}V \leq  \sigma^{(-)}_{1}U|\right\}}\mathbbm{1}_{\left\{ \sigma^{(+)}_{1}V  \leq 2s - \sigma^{(-)}_{1}U\right\}}\right. \\
& \hspace{-10cm}\left. \left(1 - \Phi_{\pi^{\scriptscriptstyle(i)}_{W}(x) }\left(\max\left(- \pi^{\scriptscriptstyle(i)}_{U}(x)U + \left(\sigma^{(+)}_{1} - \pi^{\scriptscriptstyle(i)}_{V}(x)\right)V, s - \left(\frac{1}{2}\sigma^{(-)}_{1}+ \pi^{\scriptscriptstyle(i)}_{U}(x)\right)U  +  \left(\frac{1}{2}\sigma^{(+)}_{1}- \pi^{\scriptscriptstyle(i)}_{V}(x)\right)V\right)\right) \right)\right)\\
& \hspace{-10cm}+ \mathbb{E}\left(\mathbbm{1}_{\left\{|2t - \sigma^{(+)}_{1}V \leq  \sigma^{(-)}_{1}U|\right\}}\mathbbm{1}_{\left\{ \sigma^{(+)}_{1}V  \geq 2s + \sigma^{(-)}_{1}U\right\}}\right. \\
& \hspace{-10cm}\left. \Phi_{\pi^{\scriptscriptstyle(i)}_{W}(x) }\left(\min\left(- \pi^{\scriptscriptstyle(i)}_{U}(x)U + \left(\sigma^{(+)}_{1} - \pi^{\scriptscriptstyle(i)}_{V}(x)\right)V, s - \left(\frac{1}{2}\sigma^{(-)}_{1}+ \pi^{\scriptscriptstyle(i)}_{U}(x)\right)U  +  \left(\frac{1}{2}\sigma^{(+)}_{1}- \pi^{\scriptscriptstyle(i)}_{V}(x)\right)V\right)\right) \right)\\\
\end{align*}
}
